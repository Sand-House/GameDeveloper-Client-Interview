...
여기서 뮤텍스는 사실상 Binary Semaphore라고 볼 수 있습니다. 다만 다음과 같은 차이점이 있습니다.
- 뮤텍스 : 소유권이 있음, 즉 lock을 건 스레드만 unlock 가능. 목적은 임계 구역 보호.
- 이진 세마포어 : 소유권이 없음, 즉 wait한 스레드 ≠ signal하는 스레드여도 됨, 목적은 동기화/타이밍 제어.
