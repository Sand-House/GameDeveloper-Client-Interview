...
page fault가 과도하게 발생하면 메모리와 디스크 상에서 페이지를 이동시키는 작업이 많아지면서 시스템이 급격하게 저하되는데 이 현상을 thrashing이라고 합니다.

CPU 메모리 접근 -> MMU가 페이지 테이블 검사 -> 페이지 없음 -> 커널 모드 진입 -> 페이지 폴트 핸들러 실행
